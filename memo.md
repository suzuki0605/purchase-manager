...
### 購入リスト
- カテゴリフィルター・期間指定・並び替え（新しい順/古い順のみ）
- 月別区切り表示：「2026年6月の購入」ヘッダーが自動挿入される
...
- 編集時の画像処理：imageKey（R2キー）を引き継ぎ、新画像選択時のみR2アップロード＆PUT
...
- フォント：システムフォント
- カード：3列グリッド
- 月別区切り：.month-divider（grid-column: 1/-1、グレー文字、下線）
...
- タブスワイプは左端30px・右端30px除外（パネル閉じる・進むと競合しないよう）
